...
export function AppSidebar({className = ""}: AppSidebarProps) {
    ...
    const menuItems = [
        ...
        {title: "Lịch sử thanh toán", icon: LineChart, badge: null},
    ...
    if (!isClient) {
        return (
            <Sidebar className={`z-10 w-64 h-[80vh] bg-gray-100 border-r-0 ${className}`}>
                            ...
                            {menuItems.map((item, index) => (
                                <SidebarMenuItem
                                    ...
                                    className={`flex justify-between items-center px-3 py-3 rounded-xl transition-colors cursor-pointer ${
                                        activeItem === item.title ? "bg-white shadow-md" : "hover:bg-slate-500/60"
                                    }`}
