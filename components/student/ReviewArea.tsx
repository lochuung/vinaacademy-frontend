  ...
  if (isLoading) {
    return (
      <div className="p-6 flex justify-center items-center h-64">
        <div className="animate-pulse text-gray-500">Đang tải đánh giá...</div>
      </div>
    );
  }
          ...
          <div className="mt-6 space-y-2" aria-label="Phân bố đánh giá">
            {[5, 4, 3, 2, 1].map((stars, index) => (
              <div key={stars} className="flex items-center gap-2">
                ...
                />
                <span className="text-xs text-gray-600 w-10">{Math.round(ratingPercentages[index])}%</span>
              </div>
            ))}
          </div>
        </div>
                ...
                <article key={review.id} className="border-b pb-6 last:border-b-0">
                  <header className="flex items-center gap-4 mb-2">
                    ...
                    <div className="flex-1">
                      <div className="font-medium">{review.user?.name} {review.userId === currentUserId? "(Bạn)":""}</div>
                      <div className="flex items-center gap-2">
                        ...
                        <time className="text-xs text-gray-600" dateTime="2023-10-10">{review.createdAt}</time>
                      </div>
                    </div>
